...
<Main>
	<Section>
		...
		<p>
			While I was looking through some code on GitHub, I found a <BlankLink
				...
				>repo</BlankLink
			>, that draws quads without having to populate a vertex buffer with
			data.
		</p>
		<p>
			The repo uses D3D11 and HLSL, so there are some small differences
	...
	<Section>
		...
		<p>
			In Javascript land the only thing that changes from a hello-triangle
		...
		<p>
			Note that we don't even need to bind a <var>VAO</var> in WebGL. In OpenGL
			you do need to bind it, but you can just create an empty one, and it
			will work.
		</p>

		<p>
			In the vertex shader <var>gl_VertexID</var> is the zero-based integer
		...
		<p>
			We can easily get the UV coordinates from this data by using a
...
`}
		...
		<p>
			Remapping the uv to a <var>[-1, 1]</var> range computes a fullscreen
			quad.
		</p>
...
vec2 pos = uv * 2.0 - 1.0;
		...
		<p>
			<var>gl.TRIANGLE_STRIP</var> is the key to making this work. This
		...
		<p>
			Other modes do not follow the same rules and vertex order, so it
		...
		<p>
			Note that this is a full-fledged quad, we can apply any transform we
			want to the newly created vertices. It also works with any number of
			quads, just make sure to use the right amount of vertices in <var
				>gl.drawArrays()</var
			>.
		</p>
	...
	<Section>
		...
		<p>
			If we are only ever going to need a fullscreen quad, there is an
...
vec2 pos = uv * 2.0 - 1.0;
		...
		<p>
			Since we only need 3 vertices for a triangle, it's not necessary to
